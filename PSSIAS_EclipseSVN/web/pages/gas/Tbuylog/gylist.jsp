	...
	<body onload="quickSelectInit()">
      ...
      <tr>
		...
		<ec:table items='page.result' var="item" retrieveRowsCallback="limit"
			...
			action="${ctx}/pages/gas/Tbuylog/rygyshow.do" autoIncludeParameters="true">
			<ec:row>
				<ec:column property="logtime" title="<%=Tbuylog.ALIAS_LOGTIME%>" />
				<mytag:lookupcolumn property="buytype"
					title="<%=Tbuylog.ALIAS_BUYTYPE%>"
					cell="net.java.dev.ec.table.view.LookUpCell"
					dictType="DIC_ITEM_BUY_TYPE" />

				<ec:column property="quantity" title="<%=Tbuylog.ALIAS_QUANTITY%>" />

				<ec:column property="name" title="<%=Tbuylog.ALIAS_CPCODE%>">
<!--					<a href="${ctx}/pages/gas/Tcompanyinfogas/cpshow.do?cpcode=${item.cpcode}"-->
...
<!--						 </a>-->
				...
				<ec:column width="10%" property="操作" title="操作" sortable="false"
					viewsDenied="xls">
					<a href="${ctx}/pages/gas/Tbuylog/gyshow.do?id=${item.id}&"> 查看</a>&nbsp;&nbsp;&nbsp;
		</ec:column>
			</ec:row>
		</ec:table>
	  <table width="100%" bordercolor="#7c8ca7" align="center"  cellPadding="0" cellSpacing="0" class="tb_all">
