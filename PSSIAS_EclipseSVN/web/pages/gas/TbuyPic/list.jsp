...
<head>
	...
	<title><%=TbuyPic.TABLE_ALIAS%> 维护</title>
</head>
...
<ec:table items='page.result' var="item" method="get"
	...
	action="${ctx}/pages/gas/TbuyPic/list.do" autoIncludeParameters="true">
	<ec:row>
		<ec:column property="选择" title="<input type='checkbox' onclick=\"setAllCheckboxState('items',this.checked)\" >" sortable="false" width="3%" viewsAllowed="html">
			<input type="checkbox" name="items" value="id=${item.id}&"/>
		</ec:column>
		                    <ec:column property="piclen"  title="<%=TbuyPic.ALIAS_PICLEN%>"/>
		                    <ec:column property="picture"  title="<%=TbuyPic.ALIAS_PICTURE%>"/>
		<ec:column property="操作" title="操作" sortable="false" viewsAllowed="html">
			<a href="${ctx}/pages/gas/TbuyPic/show.do?id=${item.id}&<mytag:params includes="ec*,s*" type="queryStringUtf"/>">查看</a>&nbsp;&nbsp;&nbsp;
			<a href="${ctx}/pages/gas/TbuyPic/edit.do?id=${item.id}&<mytag:params includes="ec*,s*" type="queryStringUtf"/>">修改</a>
		</ec:column>
	</ec:row>
</ec:table>
...
<script>
	  function doDel() {
	        ...
	        if (confirm('确定执行[删除]操作?')){
				...
				input_txt.value = "!/pages/gas/TbuyPic/list.do?<mytag:params includes="ec*,s*" type="queryStringUtf"/>";
				form.appendChild(input_txt);
		        form.action = '${ctx}/pages/gas/TbuyPic/delete.do';
	            form.submit();
	        }
	  }
</script>