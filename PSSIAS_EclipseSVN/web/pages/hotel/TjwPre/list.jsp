...
<ec:table items='page.result' var="item" method="get"
	...
	<ec:row>
		                    ...
		                    <mytag:lookupcolumn property="passT"  title="<%=TjwPre.ALIAS_PASS_T%>"	cell="net.java.dev.ec.table.view.LookUpCell" dictType="DIC_ITEM_PASSPORT" />
		                    <ec:column property="passNo"  title="<%=TjwPre.ALIAS_PASS_NO%>"/>
							<ec:column property="hotelname" title="<%=TjwPre.ALIAS_HOTELNAME%>">
								<a href="${ctx}/pages/hotel/Thotel/show.do?code=${item.hotelid}" >${item.hotelname}</a>
							</ec:column>		                    
		                    <ec:column property="noRoom"  title="<%=TjwPre.ALIAS_NO_ROOM%>"/>
		                    <ec:column property="inTime"  title="<%=TjwPre.ALIAS_IN_TIME%>" parse="yyyyMMddHHmm" format="yyyy-MM-dd HH:mm" cell="date"/>
		<ec:column property="操作" title="操作" sortable="false" viewsAllowed="html">
			<a href="${ctx}/pages/hotel/TjwPre/show.do?id=${item.id}&<mytag:params includes="ec*,s*" type="queryStringUtf"/>">查看</a>&nbsp;&nbsp;&nbsp;
		</ec:column>
	</ec:row>
</ec:table>
