...
Ext.onReady(function(){
		  ...
		  },
		  {
	      	...
	      	renderer:function(value,metadata,record,rowIndex,colIndex){
	      		...
	      		var str='<button title="删除" value=" " class="btn-del" onclick="treeDept.remove('+editId+')">&nbsp;</button>';
	      		    str+='&nbsp;<button title="编辑" value=" " class="btn-edit" onclick="AppUserViewzazh.edit('+editId+',\''+editName+'\')">&nbsp;</button>';
	      		return str;
	      	}
	      }
      	],
	...
	// Store
		var ds = new Ext.data.Store( {
			proxy : new Ext.data.HttpProxy( {
				url : '../SsDept/findUserByDepartment4zazh.do'
			}),
			//
			reader : _jsonReader
		});
  ...
  ds.load({params:{start:0, limit:15}});
    ...
    treePanel.on('beforeload', function(node) {
                ...
                treePanel.loader.dataUrl = '../SsDept/getTrees4zazh.do?parentId=' + node.id; // 定义子节点的Loader
...
treeDept.clickNode=function(node){
    if (node != null) {
    	var users = Ext.getCmp('UserView');
    	var ds=users.getStore();
    	ds.url='../SsDept/findUserByDepartment4zazh.do';
