...
<html>
	...
	<body onload="quickSelectInit()">
		...
		<div class="queryPanel">
			<s:form action="/pages/pmdd/ThotelBsQa/htadlist.do" theme="simple" name="form1"
					...
					<tr class="crosscolor_tr">
						...
						<td>
							<table class="list">
			              		<tr>
			              			...
			              			<td>从</td>
			              			<td>
			              				<input id="d3132" name="s_wtsjBegin"  value="${pageRequest.filters.wtsjBegin}"   maxlength="0" size="12" class="Wdate" type="text" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'d3142\')}'})"/>
			              			</td>
			              			<td>到</td>
			              			<td>
						...
						<table class="list">
			              		<tr>
			              			...
			              			<td>从</td>
			              			<td>
			              				<input id="d3133" name="s_jdsjBegin"  value="${pageRequest.filters.jdsjBegin}"   maxlength="0" size="12" class="Wdate" type="text" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'d3143\')}'})"/>
			              			</td>
			              			<td>到</td>
			              			<td>
					...
					<tr>
						<td class="tb_bottom" colspan="4">
							<input type="submit" value="查询"
								onclick="getReferenceForm(this).action='${ctx}/pages/pmdd/ThotelBsQa/htadlist.do'" />
		...
		<ec:table items='page.result' var="item" method="get"
			...
			filterRowsCallback="limit" action="${ctx}/pages/pmdd/ThotelBsQa/list.do"
			...
			<ec:row>
				...
				<ec:column property="jdnr" title="<%=ThotelBsQa.ALIAS_JDNR%>" />
				...
				<ec:column property="操作" title="操作" sortable="false"
					viewsAllowed="html">
					<a href="${ctx}/pages/pmdd/ThotelBsQa/htadshow.do?xh=${item.xh}&<mytag:params includes="ec*,s*" type="queryStringUtf"/>">查看</a>&nbsp;&nbsp;&nbsp;
					 <c:if test="${item.jdbz == '1'}">
						<a href="${ctx}/pages/pmdd/ThotelBsQa/htadedit.do?xh=${item.xh}&<mytag:params includes="ec*,s*" type="queryStringUtf"/>">解答</a>
					</c:if>
				</ec:column>
			</ec:row>
		</ec:table>
...
<script>
	  function doDel() {
	        ...
	        if (confirm('确定执行[删除]操作?')){
				...
				input_txt.value = "!/pages/pmdd/ThotelBsQa/list.do?<mytag:params includes="ec*,s*" type="queryStringUtf"/>";
				form.appendChild(input_txt);
		        form.action = '${ctx}/pages/pmdd/ThotelBsQa/delete.do';
	            form.submit();
	        }
	  }
</script>