...
<s:form action="/pages/pmdd/Temployee/update.do"  theme="simple" name="inputForm"  method="post">
	...
	<table cellpadding="0" cellspacing="0" border="0" class="tb_all">
	    <input type="hidden" name="returnUrl" value="!/pages/pmdd/Temployee/list.do?<mytag:params includes="ec*,s*" type="queryStringUtf"/>" />
	        <tr>
				      <td colspan="5" class="tb_title"> 
							<%=Temployee.TABLE_ALIAS%>复职
				     </td>
		    </tr>
	           <tr class="crosscolor_tr">
			              ...
			              <td>
						           <mytag:select property="%{model.sex}"   styleClass="required validate-selection"  name="sex"   notEmpty="false"  dictName="gender"/>
		                  </td>
		                   <td rowspan="11" width="18%" align="center" >
			      <%if(picCount != null && picCount.equals("1")) { %>
		             <a onclick="javascript:uploadEmpPhoto('<s:property value="%{model.empcode}" />');return false;" href="#"> <img src='${ctx}/pages/Temployee/showPic.do?xh=<s:property value="%{model.empcode}" />'  height="126" alt="" width="102" border="0" name="photo"> </a>	
		           <% } else {%>
		             <a onclick="javascript:uploadEmpPhoto('<s:property value="%{model.empcode}" />');return false;" href="#"> <IMG src="${ctx}/images/spacer.gif" height="126" alt="" width="102" border="0" name="photo"> </a>
		           <%} %>
		       </td>
                   </tr>
		           <tr class="crosscolor_tr">
		           ...
		           <tr class="crosscolor_tr">
		           		   <td class="crosscolor_td">
			                    <FONT color="red">*</FONT>  <%=Temployee.ALIAS_RZRQ%>
		                  </td>
			              <td >
						<input value="${model.rzrq}" class="required Wdate" size="16" onkeyup="DateFormat(this);" 	onclick="WdatePicker({startDate:'new Date()',dateFmt:'yyyy-MM-dd'})"
							id="rzrq" name="rzrq"  />
		                  </td>
                          <td class="crosscolor_td">
		           ...
		           <tr class="crosscolor_tr">
		                 ...
		                 <td class="crosscolor_td">
			              ...
			              <td >
		                           <s:textfield label="%{@vs@ALIAS_NOWADRESS}" size="35" key="nowadress" value="%{model.nowadress}"  cssClass="max-length-100" required="false" />
		                  </td>
...
<script>
	...
	new Validation(document.forms[0],{onSubmit:true,onFormValidate : function(result,form) {
		...
		//在这里添加自定义验证
		showBirthday();
		var cyrjzt=document.all.cyrjzt.value;
		if(cyrjzt==1){
			if (confirm('离职后将不能再修改人员信息。确认离职吗？')){
	...
	function chg_sfzh(sfzh,year)
{
   ...
   function changeprov()
	...
	function uploadEmpPhoto(ID) {
		...
		var returnvalue = window.showModalDialog("${ctx}/pages/pmdd/Temployee/uploadPhoto.jsp?xh="+ID,"childWIn","dialogHeight:150px;dialogWidth:400px;scroll:off;center:yes");
		if (returnvalue == "yes"){
		    changesrc();
	      }      
	}
...
　　　　	return Math.ceil(rnd()*number);  
   ...
   function changesrc(){
	    ...
	    pic.src='${ctx}/pages/pmdd/Temployee/showPic.do?xh=<s:property value="%{model.empcode}" />&rand='+rand(1000);
   }  
