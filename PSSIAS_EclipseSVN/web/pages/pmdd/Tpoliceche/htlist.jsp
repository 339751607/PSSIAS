	    ...
	    <table cellpadding="0" cellspacing="0" border="0" class="tb_all">
		           ...
		           <tr class="crosscolor_tr">
			              ...
			              <table class="list">
			              		<tr>
			              			<td>
			              				<s:select name="dateSelect4" list="dateSelectMap"  onchange="dateselect(this,'s_checktimeBegin','s_checktimeEnd','yyyy-MM-dd HH:mm:ss');"  value="#request.dateSelect4" listKey="key"   listValue="value" theme="simple" label=""  emptyOption="false" ></s:select>
			              			</td>
			              			<td>从</td>
			              			<td>
			              				<input id="d3134" name="s_checktimeBegin"  value="${pageRequest.filters.checktimeBegin}"   maxlength="0" size="12" class="Wdate" type="text" onFocus="WdatePicker({dateFmt:'<%=Tpoliceche.FORMAT_DEADLINE%>',maxDate:'#F{$dp.$D(\'d3144\')}'})"/>
			              			</td>
			              			<td>到</td>
			              			<td>
			              				<input id="d3144" name="s_checktimeEnd"   value="${pageRequest.filters.checktimeEnd}"  maxlength="0" size="12" class="Wdate" type="text" onFocus="WdatePicker({dateFmt:'<%=Tpoliceche.FORMAT_DEADLINE%>',minDate:'#F{$dp.$D(\'d3134\')}'})"/>
			              			</td>
			              		</tr>
			              	</table>       
		                  </td>
			              ...
			              <table class="list">
			              		<tr>
			              			<td>
			              				<s:select name="dateSelect36" list="dateSelectMap"  onchange="dateselect(this,'s_deadlineBegin','s_deadlineEnd','yyyy-MM-dd HH:mm:ss');"  value="#request.dateSelect36" listKey="key"   listValue="value" theme="simple" label=""  emptyOption="false" ></s:select>
			              			</td>
			              			<td>从</td>
			              			<td>
			              				<input id="d31336" name="s_deadlineBegin"  value="${pageRequest.filters.deadlineBegin}"   maxlength="0" size="12" class="Wdate" type="text" onFocus="WdatePicker({dateFmt:'<%=Tpoliceche.FORMAT_DEADLINE%>',maxDate:'#F{$dp.$D(\'d31436\')}'})"/>
			              			</td>
			              			<td>到</td>
			              			<td>
...
<ec:table items='page.result' var="item" method="get"
	...
	action="${ctx}/pages/pmdd/Tpoliceche/htlist.do" autoIncludeParameters="true">
	<ec:row>
		
		                    <ec:column property="companyinfo"  title="<%=Tpoliceche.ALIAS_COMPANYINFO%>"/>
		...
		<ec:column property="操作" title="操作" sortable="false" viewsAllowed="html">
			<a href="${ctx}/pages/pmdd/Tpoliceche/show.do?checkid=${item.checkid}&<mytag:params includes="ec*,s*" type="queryStringUtf"/>">查看</a>&nbsp;&nbsp;&nbsp;
...
<script>
	  function doDel() {
	        ...
	        if (confirm('确定执行[删除]操作?')){
				...
				input_txt.value = "!/pages/pmdd/Tpoliceche/list.do?<mytag:params includes="ec*,s*" type="queryStringUtf"/>";
				form.appendChild(input_txt);
		        form.action = '${ctx}/pages/pmdd/Tpoliceche/delete.do';
	            form.submit();
	        }
	  }
</script>