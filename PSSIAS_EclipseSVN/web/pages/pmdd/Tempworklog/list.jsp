...
<ec:table items='page.result' var="item" method="get"
	...
	action="${ctx}/pages/pmdd/Tempworklog/list.do" autoIncludeParameters="true">
	<ec:row>
		...
		<ec:column property="操作" title="操作" sortable="false" viewsAllowed="html">
			<a href="${ctx}/pages/pmdd/Tempworklog/show.do?worklogid=${item.worklogid}&<mytag:params includes="ec*,s*" type="queryStringUtf"/>">查看</a>&nbsp;&nbsp;
			
			<c:if test="${item.status=='0'}">
			<a href="${ctx}/pages/pmdd/Tempworklog/edit.do?worklogid=${item.worklogid}&<mytag:params includes="ec*,s*" type="queryStringUtf"/>">修改</a>
			</c:if>
		</ec:column>
	</ec:row>
</ec:table>
...
<script>
	  function doDel() {
	        ...
	        if (confirm('确定执行[删除]操作?')){
				...
				input_txt.value = "!/pages/pmdd/Tempworklog/list.do?<mytag:params includes="ec*,s*" type="queryStringUtf"/>";
				form.appendChild(input_txt);
		        form.action = '${ctx}/pages/pmdd/Tempworklog/delete.do';
	            form.submit();
	        }
	  }
</script>