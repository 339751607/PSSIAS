...
<head>
	...
	<title><%=TcpCheck.TABLE_ALIAS%> 维护</title>
</head>
...
<div class="queryPanel">
    <s:form action="/pages/fjy/TcpCheck/list.do"  name="inputForm"   theme="simple" style="display: inline;" method="post">
		           ...
		           <tr class="tr_tb">
			              ...
			              <td colspan="3" class="td_input">
				                   <s:select name="dateSelect2" list="dateSelectMap"  onchange="dateselect('list_dateSelect2','d3132','d3142','yyyy-MM-dd');"  value="#request.dateSelect2" listKey="key"   listValue="value" theme="simple" label=""  emptyOption="false" ></s:select>
			                          从<input id="d3132" name="s_checktimeBegin"  value="${pageRequest.filters.checktimeBegin}"   maxlength="0" size="12" class="Wdate" type="text" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'d3142\')}'})"/> &nbsp;到&nbsp;
			                        <input id="d3142" name="s_checktimeEnd"   value="${pageRequest.filters.checktimeEnd}"  maxlength="0" size="12" class="Wdate" type="text" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'d3132\')}'})"/>
		                  </td>
...
<ec:table items='page.result' var="item" method="get"
	...
	action="${ctx}/pages/fjy/TcpCheck/list.do" autoIncludeParameters="true">
	<ec:row>
		<ec:column property="选择" title="<input type='checkbox' onclick=\"setAllCheckboxState('items',this.checked)\" >" sortable="false" width="3%" viewsAllowed="html">
			<input type="checkbox" name="items" value="id=${item.id}&"/>
		</ec:column>
		                    <ec:column property="deptname"  title="部门名"/>
		                    <ec:column property="idcard"  title="<%=TcpCheck.ALIAS_IDCARD%>"/>
				            <ec:column property="checktime"  parse="yyyyMMddHHmmss" format="yyyy-MM-dd HH:mm:ss" cell="date"  title="<%=TcpCheck.ALIAS_CHECKTIME%>"/>
		                    
		<ec:column property="操作" title="操作" sortable="false" viewsAllowed="html">
			<a href="${ctx}/pages/fjy/TcpCheck/tab.do?checkid=${item.id}&<mytag:params includes="ec*,s*" type="queryStringUtf"/>">查看</a>&nbsp;&nbsp;&nbsp;
...
<script>
	  function doDel() {
	        ...
	        if (confirm('确定执行[删除]操作?')){
				...
				input_txt.value = "!/pages/fjy/TcpCheck/list.do?<mytag:params includes="ec*,s*" type="queryStringUtf"/>";
				form.appendChild(input_txt);
		        form.action = '${ctx}/pages/fjy/TcpCheck/delete.do';
	            form.submit();
	        }
	  }
