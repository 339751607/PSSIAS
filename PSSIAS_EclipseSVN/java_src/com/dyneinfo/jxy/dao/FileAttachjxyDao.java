...
@Component
public class FileAttachjxyDao extends BaseSpringJdbcDao<FileAttach, Long> {
	...
	public List getFileContent(final String FILEID,String sqlWhere) {
		String sql = "select FILEID,FILENAME,CONTENT,FILE_SAVE,FILEGROUP,RELATION_ID  from FILE_ATTACH " +
				" where FILEID = ?   ";
		if(sqlWhere != null && sqlWhere.length() > 0)
			sql = sql + sqlWhere;
		return getJdbcTemplate().query(sql, new String[] { FILEID },
