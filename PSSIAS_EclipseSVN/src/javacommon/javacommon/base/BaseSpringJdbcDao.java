...
public abstract class BaseSpringJdbcDao<E,PK extends Serializable> extends JdbcDaoSupport implements EntityDao<E,PK>{
	...
	public Page pageQuery1(String query,PageRequest pageRequest) {
		return pageQuery(query,pageRequest,new BeanPropertyRowMapper(getEntityClass()), "select count(distinct  t.NOTICEID) ");
	}
	...
	public Page pageGroupQuery(String query, String countQuery,
			final PageRequest pageRequest, RowMapper rowMapper) {

		final int totalCount = getSimpleJdbcTemplate().queryForInt(countQuery);
		...
		List list = (List) getNamedParameterJdbcTemplate().queryForList(querySql, otherFilters);
		page.setResult(list);
		return page;
	}
